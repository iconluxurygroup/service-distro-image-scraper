...
async def update_search_sort_order(
    ...
    try:
        ...
        async with engine_ctx as conn:
            ...
            # Fetch results with explicit columns
            query = text("""
                SELECT 
                    t.ResultID, 
                    t.EntryID,
            ...
            """)
            logger.debug(f"Executing query: {query} with parameters: file_id={file_id}, entry_id={entry_id}")
            ...
            try:
                ...
                logger.error(f"SQL execution failed for FileID {file_id}, EntryID {entry_id}: {e}", exc_info=True)
                logger.debug(f"Query: {query}, Parameters: file_id={file_id}, entry_id={entry_id}")
                return None
...
def get_images_excel_db(file_id: str, logger: Optional[logging.Logger] = None) -> pd.DataFrame:
    """
    Fetch images for Excel export from the database for a given file_id.
    Returns a DataFrame with columns: ExcelRowID, ImageUrl, ImageUrlThumbnail, Brand, Style, Color, Category.
    """
    logger = logger or logging.getLogger(__name__)
    ...
    try:
        ...
        with pyodbc.connect(conn_str, timeout=10) as conn:
            ...
            # Filter out rows with missing ImageUrl
