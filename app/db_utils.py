...
async def update_search_sort_order(
    ...
    try:
        ...
        async with async_engine.connect() as conn:
            ...
            async with conn.begin():
                ...
                if not all([brand, model]):
                    ...
                    result.close()  # Explicitly close cursor
                    if row:
                ...
                # Bulk update SortOrder using a single query
            ...
            # Verify updates in a separate transaction
            async with conn.begin():
                result = await conn.execute(
                    text("""
                        SELECT ResultID, EntryID, SortOrder, ImageDesc, ImageSource, ImageUrl
        ...
        # Fallback: Set SortOrder to -2 to avoid NULL values
        async with async_engine.connect() as conn:
            await conn.execute(
