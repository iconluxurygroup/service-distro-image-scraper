...
def decode_url(url: str, logger: Optional[logging.Logger] = None) -> str:
    ...
    try:
        ...
        is_google_thumb = 'tbn:' in cleaned or 'tbn%3A' in cleaned or 'tbn' in cleaned
        if is_google_thumb:
            logger.debug("Detected Google thumbnail URL")
            # Parse URL components
            parsed = urllib.parse.urlparse(cleaned)
            query_dict = urllib.parse.parse_qs(parsed.query)
            # Extract and fix thumbnail ID
            if 'q' in query_dict:
                q_values = query_dict['q']
                fixed_q = []
                for q in q_values:
                    q = q.replace('q\\=tbn', 'tbn').replace('q\=tbn', 'tbn').replace('tbn\\:', 'tbn:')
            ...
            # Remove backslashes from other query parameters
            for key, values in query_dict.items():
                query_dict[key] = [v.replace('\\', '') for v in values]
            # Rebuild query
            query = urllib.parse.urlencode(query_dict, doseq=True) if query_dict else ''
            # Reconstruct URL
