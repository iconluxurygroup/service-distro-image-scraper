...
def get_s3_client(service='s3', logger=None, file_id=None):
    ...
    try:
        ...
        config = AioConfig(
            ...
            signature_version='s3v4'  # Explicitly use Signature Version 4
        )
        if service == 'r2':
            logger.debug(f"R2 endpoint: {S3_CONFIG['r2_endpoint']}, access_key: {S3_CONFIG['r2_access_key'][:4]}...")
...
def double_encode_plus(filename, logger=None):
    """Double-encode filenames, handling '+' and other special characters."""
    logger = logger or default_logger
    logger.debug(f"Encoding filename: {filename}")
    first_pass = filename.replace('+', '%2B')
    second_pass = urllib.parse.quote(first_pass, safe='')
...
async def upload_file_to_space(file_src, save_as, is_public=True, logger=None, file_id=None):
    """Upload a file to S3 and R2 asynchronously, with retry logic."""
    logger = logger or default_logger
    if logger == default_logger and file_id:
        logger, _ = setup_job_logger(job_id=file_id, console_output=True)
    ...
    for attempt in range(max_attempts):
        try:
            async with get_s3_client(service='s3', logger=logger, file_id=file_id) as s3_client:
                ...
                logger.info(f"Uploaded {file_src} to S3 in {time.time() - start_time:.2f} seconds")
                ...
                logger.info(f"S3 URL: {s3_url} with Content-Type: {content_type}")
                result_urls['s3'] = s3_url
                break
        except ClientError as e:
            if attempt < max_attempts - 1 and e.response['Error']['Code'] == 'InternalError':
                logger.warning(f"InternalError on attempt {attempt + 1}, retrying after {2 ** attempt}s...")
    ...
    for attempt in range(max_attempts):
        try:
            async with get_s3_client(service='r2', logger=logger, file_id=file_id) as r2_client:
                ...
                start_time = time.time()
                with open(file_src, 'rb') as file:
                    await r2_client.upload_fileobj(
                        Fileobj=file,
                ...
                logger.info(f"Uploaded {file_src} to R2 in {time.time() - start_time:.2f} seconds")
                double_encoded_key = double_encode_plus(save_as, logger=logger)
                r2_url = f"{S3_CONFIG['r2_custom_domain']}/{double_encoded_key}"
                logger.info(f"R2 URL: {r2_url} with Content-Type: {content_type}")
                result_urls['r2'] = r2_url
                break
        except ClientError as e:
            error_code = e.response['Error']['Code']
            if attempt < max_attempts - 1 and error_code in ['SignatureDoesNotMatch', 'InternalError']:
                logger.warning(f"{error_code} on attempt {attempt + 1}, retrying after {2 ** attempt}s...")
                await asyncio.sleep(2 ** attempt)  # Exponential backoff
                continue
