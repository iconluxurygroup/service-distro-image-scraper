...
async def process_search_row(search_string, endpoint, entry_id, attempt=1):
    ...
    try:
        ...
        logger.error(f"Error making request: {e}. Attempting retry...")
        remove_endpoint(endpoint)
        n_endpoint = get_endpoint()
        if attempt < MAX_RETRIES:
            return await process_search_row(search_string, n_endpoint, entry_id, attempt + 1)
...
def extract_domains_and_counts(data):
    ...
    print(f"Extracted domains: {domains}")  # 🔍 Debugging line
    domain_counts = Counter(domains)
    print(f"Domain counts: {domain_counts}")  # 🔍 Debugging line
