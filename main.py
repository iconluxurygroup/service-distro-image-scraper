...
"""
    ...
    if (to_emails == cc_recipient) or (to_emails == 'No Email Found'):
    ...
    try:
        #
        sg = SendGridAPIClient('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')
...
def zero_shot_classify(image_url):
    try:
        model = CLIPModel.from_pretrained("openai/clip-vit-large-patch14-336")
        processor = CLIPProcessor.from_pretrained("openai/clip-vit-large-patch14-336")
        texts = ["Clothing", "Footwear", "Bags", "Jewelry", "Hats & Headwear", "Eyewear", "Belts", "Scarves & Gloves",
                 "Beauty & Fragrances", "Parts"]
        if 'No google' in image_url:
            logging.info("X*X*X*X*X*X*X*X*XX*X*X*XX*")
            return None, None
        # Fetch and open image with error handling
        ...
        image = IMG2.open(image_data)
        image.verify()  # Verify image without loading it fully

        # Reopen the image for further processing after verification
        image_data.seek(0)  # Reset stream pointer to the beginning
        image = IMG2.open(image_data)
        ...
        results = [{"text": text, "probability": prob.item()} for text, prob in zip(texts, probs[0])]
        results_json = json.dumps(results, indent=2)

        highest_caption = max(results, key=lambda x: x["probability"])["text"]
        return results_json, highest_caption

    except UnidentifiedImageError:
        print(f"Could not identify image from URL: {image_url}")
        return None, None
...
async def generate_download_file(file_id):
    ...
    selected_image_list = await loop.run_in_executor(ThreadPoolExecutor(), prepare_images_for_download_dataframe,
                                                     selected_images_df)
    ...
    response = await loop.run_in_executor(None, requests.get, provided_file_path,
    ...
    failed_rows = await loop.run_in_executor(ThreadPoolExecutor(), write_excel_image, local_filename, temp_images_dir,
                                             preferred_image_method)
    ...
    if failed_img_urls:
        ...
        fail_rows_written = await loop.run_in_executor(ThreadPoolExecutor(), write_failed_downloads_to_excel,
                                                       failed_img_urls, local_filename)
    ...
    is_public = True
    public_url = await loop.run_in_executor(ThreadPoolExecutor(), upload_file_to_space, local_filename, file_name,
                                            is_public)
    ...
    return {"message": "Processing completed successfully.", "results": 'hardcoded result value',
...
def process_db_row(row):
    ...
    if 'No google' in searchString:
        logger.info(f"Entry Id: {entry_id}\nSearch String {searchString}")
        logger.info('need to skip??')
    else:
        logger.info(f"Entry Id: {entry_id}\nSearch String {searchString}")
...
async def download_all_images(data, save_path):
    ...
    async with RetryClient(raise_for_status=False, retry_options=retry_options, timeout=timeout,
        ...
        try:
            async with session.get(url, headers=headers) as response:
                ...
                if response.status == 200:
                    ...
                    image = IMG2.open(image_data)
                    image.verify()  # Verify the image without fully loading it
...
if __name__ == "__main__":
    ...
    print(os.environ)
    # uvicorn.run("main:app", port=8090, host='0.0.0.0', reload=True)
